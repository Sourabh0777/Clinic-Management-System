...
const createInitialSchedule = async (req, res, next) => {
   ...
   try {
      ...
      const formatStartTime = moment(startTime, inputFormat).format(
         outputFormat
      );
      ...
      for (let i = 0; i < upcoming30Dates.length; i++) {
         for (let j = 0; j < workingDays.length; j++) {
            if (
               upcoming30Dates[i].toLocaleString("en-us", {
                  weekday: "long",
               }) == workingDays[j]
            ) {
      ...
      for (let i = 0; i < next30WorkingDates.length; i++) {
         ...
         const slotEndTimeAndDate = moment(
            next30WorkingDates[i].endDateAndTime
         );
